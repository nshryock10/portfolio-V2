...
export default function Nav({}: Props) {
  ...
  return (
    <div  className='fixed top-0 z-50 h-screen box-border overflow-hidden'>
        <button onClick={() => setIsOpen(!isOpen)} className='h-[20px] flex flex-col justify-around absolute z-10 top-10 left-10
        md:hidden' > 
            <div className={`px-[17px] py-[1px] rounded-md bg-black rotate-0 transition-transform
             ${isOpen ? "rotate-[45deg]" : "rotate-0"}`} />
            <div className={`px-[12px] py-[1px] rounded-md bg-black rotate-0 transition-transform origin-[3.7px_-2.5px]
        ...
        md:left-0  md:w-[var(--md-nav-width)] lg:w-[var(--lg-nav-width)] md:justify-around`}>
